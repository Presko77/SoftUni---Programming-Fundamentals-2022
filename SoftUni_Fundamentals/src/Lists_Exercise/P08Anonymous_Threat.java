...
public class P08Anonymous_Threat {
    public static void main(String[] args) {
        ...
        while (!command.equals("3:1")) {
            if (command.contains("merge")) {
                ...
                String elementsForDivide = texts.get(index);
                texts.remove(index);

                int partSize = elementsForDivide.length() / parts;
                int beginIndexOfText = 0;
                for (int part = 1; part < parts ; part++) {
                    texts.add(index,elementsForDivide.substring(beginIndexOfText, beginIndexOfText + partSize));
                    index++;
                    beginIndexOfText +=partSize;
                }
                texts.add(index,elementsForDivide.substring(beginIndexOfText));

            }

            command= scanner.nextLine();
        }
        System.out.println(texts.toString().replaceAll("[\\[\\],]", ""));
    }
}
